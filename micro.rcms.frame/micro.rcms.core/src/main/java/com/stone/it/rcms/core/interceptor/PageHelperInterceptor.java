...
public class PageHelperInterceptor implements Interceptor {
    ...
    public static PageVO getPageRequestParams(MappedStatement mappedStatement, Object param) {
        ...
        if (param instanceof ParamMap paramMap) {
    ...
    private PageResult getQueryResultData(Invocation invocation, Object[] iArgs, MappedStatement ms, Object params,
        PageVO pageVO) throws SQLException {
    ...
    private void buildPageQuery(Object params, PageVO pageVO) {
        if (params instanceof ParamMap paramMap) {
            Iterator iterator = paramMap.entrySet().iterator();
            while (iterator.hasNext()) {
                Entry entry = (Entry)iterator.next();
                Object object = entry.getValue();
