...
CREATE TABLE IF NOT EXISTS `tpl_mifi_device_t` (
  ...
  `enterprise_id` VARCHAR(100) NOT NULL COMMENT '商户ID',
  `device_sn` VARCHAR(100) NOT NULL COMMENT '设备sn',
  `is_current` VARCHAR(10) NOT NULL COMMENT '是否当前设备:1 是 0否',
  `current_iccid` VARCHAR(10) NOT NULL COMMENT '当前设备使用ICCID',
  ...
  `electric` VARCHAR(100)  COMMENT '电量',
...
CREATE TABLE IF NOT EXISTS `tpl_mifi_data_plan_t` (
  ...
  `data_plan_flow` DECIMAL(10, 2) NOT NULL COMMENT '套餐流量GB',
  `data_plan_void_flow` DECIMAL(10, 2) NOT NULL COMMENT '套餐虚量GB',
...
CREATE TABLE IF NOT EXISTS `tpl_mifi_device_dp_t` (
  ...
  `device_dp_name` VARCHAR(1000) NOT NULL COMMENT '设备套餐名称',
  `device_sn` VARCHAR(100) NOT NULL COMMENT '设备sn',
  `order_no` VARCHAR(100) NOT NULL COMMENT '订单号',
  `data_plan_no` VARCHAR(100) NOT NULL COMMENT '套餐编号',
  `effective_time` datetime NOT NULL COMMENT '生效时间',
  `expire_time` datetime NOT NULL COMMENT '失效时间',
  `total_flow` VARCHAR(100) NOT NULL COMMENT '套餐流量GB',
  `remain_flow` VARCHAR(5000) NOT NULL COMMENT '套餐剩余量GB',
  `used_flow` VARCHAR(5000) NOT NULL COMMENT '当前已用量MB',
  `total_used_flow` VARCHAR(5000) NOT NULL COMMENT '当前总共已用量MB',
