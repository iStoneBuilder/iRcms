...
public interface IDeviceRealNameService {
    ...
    PageResult<SimNameVO> findSimNamePageResult(@QueryParam("") SimNameVO simNameVO, @PathParam("") PageVO pageVO);
    ...
    int syncSimName(@PathParam("iccid") String iccid, SimNameVO simNameVO);

    @DELETE
    @Path("/records/{iccid}/clean")
    @RcmsMethod(name = "设备实名.清理")
    @RequiresPermissions("permission:device-name:clean")
    int cleanSimName(@PathParam("iccid") String iccid, SimNameVO simNameVO);
}
