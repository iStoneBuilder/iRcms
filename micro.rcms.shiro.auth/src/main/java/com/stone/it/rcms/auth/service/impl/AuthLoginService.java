...
public class AuthLoginService implements IAuthLoginService {
  ...
  public LoginResVO userLogin(AuthUserVO userVO) {
    ...
    String accessToken = buildJwtToken(sessionId, userVO.getUserId(), userVO.getPassword(), "app",
      expTime,
  ...
  public LoginResVO userLoginRefresh(LoginResVO loginResVO) {
    ...
    try {
      ...
      newLoginResVO = new LoginResVO(resVO.getAccessToken(), resVO.getRefreshToken(),
        resVO.getExpires());
    } catch (Exception e) {
      throw e;
    }
  ...
  public JSONObject appToken(AppSecretVO appSecretVO) {
    ...
    String accessToken = buildJwtToken(sessionId, appSecretVO.getAppId(), appSecretVO.getSecret(),
      "app", expTime,
      dbUser.getEnterpriseId());
  ...
  private String buildJwtToken(String sessionId, String account, String password, String type,
    Calendar instance,
    String enterpriseId) {
  ...
  public JSONObject userLogout() {
    ...
    try {
      ...
      e.printStackTrace();
      return ResponseUtil.responseBuild(HttpStatus.SC_INTERNAL_SERVER_ERROR, "退出失败！",
        e.getMessage());
    }
  }
