...
CREATE TABLE IF NOT EXISTS `tpl_fram_pay_t` (
  ...
  `pay_way` VARCHAR(100) NOT NULL COMMENT '支付配置名称（支付宝，，平台）',
  ...
  `mch_id` VARCHAR(1000) COMMENT '商户号',
  `mch_pay_v2key` VARCHAR(1000)  COMMENT '支付V2秘钥',
  `mch_pay_v3key` VARCHAR(1000)  COMMENT '支付V3秘钥',
  ...
  `mch_pay_private_key` VARCHAR(1000) COMMENT '支付商户私钥',
  `mch_pay_cert` VARCHAR(1000)  COMMENT '支付商户证书',
  `cert_path` VARCHAR(1000)  COMMENT '支付商户证书路径',
...
CREATE TABLE IF NOT EXISTS `tpl_fram_order_t` (
  ...
  `order_status` varchar(100) COMMENT '订单状态 (待支付1、已支付2、已取消3、支付失败4)',
  `refund_amount` DECIMAL(10, 2) COMMENT '退款金额 (单位：元)',
  `refund_time` datetime COMMENT '退款时间',
  `refund_status` varchar(100) COMMENT '退款状态 (申请中、已退款、退款失败)',
  `pay_type` varchar(100) COMMENT '支付方式 (、支付宝，平台代付)',
  `pay_source` varchar(100) COMMENT '支付来源 (APP、PC、小程序)',
  `pay_by` varchar(100) NOT NULL COMMENT '支付人 (用户id)',
  `buy_num` VARCHAR(100) COMMENT '购买数量',
  `transaction_id` VARCHAR(100) COMMENT '支付系统生成的订单号',
  `CREATED_TIME` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `CREATED_BY` varchar(100) NOT NULL DEFAULT 'UNKNOWN',
  `UPDATED_TIME` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `UPDATED_BY` varchar(100) NOT NULL DEFAULT 'UNKNOWN',
